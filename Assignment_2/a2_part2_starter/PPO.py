...
def train(S,A,returns, old_log_probs):
    ...
    ###############################
    # Value function update (fit V to returns) and compute advantage
    OPT1.zero_grad()
    n = torch.arange(S.size(0)).to(DEVICE)
    V_pred = V(S).view(-1)
    v_loss = (((returns - V_pred) ** 2) * (GAMMA ** n)).sum()
    v_loss.backward()
    OPT1.step()

    with torch.no_grad():
        advantage = returns - V(S).view(-1)
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        # Use gamma^n weighting similar to others
        weighted_adv = (GAMMA ** n) * advantage.detach()
        surrogate1 = ratio * weighted_adv
        surrogate2 = clipped_ratio * weighted_adv
        objective = -torch.min(surrogate1, surrogate2).sum()
        objective.backward()
        OPT2.step()
    #################################
