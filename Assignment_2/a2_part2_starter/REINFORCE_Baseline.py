...
def train(S,A,returns):
    ###############################
    # Value-function update (fit V to returns)
    # Weighted by gamma^n to match slide notation
    OPT1.zero_grad()
    ...
    # Policy gradient with baseline (advantage = returns - V)
    with torch.no_grad():
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        objective = -((GAMMA ** n) * advantage * log_probs).sum()
        objective.backward()
        OPT2.step()
    #################################
