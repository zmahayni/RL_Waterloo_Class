...
print("Iterations:", nIterations, "Final epsilon:", epsilon)
print()
...
for m in range(1, 11):
    policy_mpi, V_mpi, nIter_mpi, eps_mpi = mdp.modifiedPolicyIteration(
        np.zeros(mdp.nStates, dtype=int),
        np.zeros(mdp.nStates),
        nEvalIterations=m,
        tolerance=0.01
    )
    print(f"Partial eval iters = {m}: Iterations = {nIter_mpi}")
