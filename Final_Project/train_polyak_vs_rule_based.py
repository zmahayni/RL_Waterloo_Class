"""Training script for DQN with Polyak target updates on Leduc Hold'em."""
...
def plot_learning_curve(run_dir, window=200):
    ...
    ax.set_title(
        "Leduc Hold'em: DQN with Polyak Target Updates", fontsize=14, fontweight="bold"
    )
...
def train_polyak_dqn(args):
    """Train DQN with Polyak target updates on Leduc Hold'em."""
    # Set seed
    set_seed(args.seed)
    ...
    run_dir = Path(f"runs/leduc/polyak/{run_name}")
    ...
    # Initialize DQN agent with Polyak updates
    ...
    print(f"Target update mode: {args.target_update_mode}")
    if args.target_update_mode == "polyak":
...
def main():
    parser = argparse.ArgumentParser(
        description="Train DQN with Polyak updates on Leduc Hold'em"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Hard update frequency (only for hard mode)",
    )
