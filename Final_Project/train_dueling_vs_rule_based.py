"""Training script for Dueling DQN on Leduc Hold'em."""
...
from dqn_agent_dueling import DQNAgentDueling
...
def plot_learning_curve(run_dir, window=200):
    ...
    ax.set_title(
        "Leduc Hold'em: Dueling DQN Learning Curve", fontsize=14, fontweight="bold"
    )
...
def train_dueling_dqn(args):
    """Train Dueling DQN on Leduc Hold'em."""
    # Set seed
    set_seed(args.seed)
    ...
    run_dir = Path(f"runs/leduc/dueling/{run_name}")
...
def main():
    parser = argparse.ArgumentParser(description="Train Dueling DQN on Leduc Hold'em")
