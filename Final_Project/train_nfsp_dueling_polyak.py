"""Training script for NFSP with Dueling DQN + Polyak on Leduc Hold'em (self-play)."""
...
from nfsp_agent_dueling_polyak import NFSPAgentDuelingPolyak
...
def plot_learning_curve(run_dir, window=200):
    ...
    ax.set_title(
        "Leduc Hold'em: NFSP Dueling+Polyak Learning Curve",
        fontsize=14,
        fontweight="bold",
    )
...
def train_nfsp(args):
    ...
    run_dir = Path(f"runs/leduc/nfsp_dueling_polyak/{run_name}")
    ...
    # Initialize two NFSP agents with Dueling + Polyak for self-play
    agent_names = ["player_0", "player_1"]
    agents = {
        "player_0": NFSPAgentDuelingPolyak(
        ...
        "player_1": NFSPAgentDuelingPolyak(
            state_size=state_size,
            action_size=action_size,
            rl_lr=args.rl_lr,
            sl_lr=args.sl_lr,
    ...
    print(f"RL component: Dueling DQN + Polyak (tau={args.tau})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train NFSP with Dueling DQN + Polyak on Leduc Hold'em"
    )
    ...
    parser.add_argument(
        "--rl_lr", type=float, default=1e-4, help="Learning rate for RL"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.999,
        help="Polyak averaging coefficient for target network",
    )
    parser.add_argument(
        "--rl_buffer_size", type=int, default=100000, help="RL replay buffer size"
    )
    parser.add_argument(
