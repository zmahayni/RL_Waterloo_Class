"""Training script for Dueling DQN with Polyak updates on Leduc Hold'em."""
...
from dqn_agent_dueling_polyak import DQNAgentDuelingPolyak
...
def plot_learning_curve(run_dir, window=200):
    ...
    ax.set_title(
        "Leduc Hold'em: Dueling DQN + Polyak Learning Curve",
        fontsize=14,
        fontweight="bold",
    )
...
def train_dueling_polyak_dqn(args):
    """Train Dueling DQN with Polyak updates on Leduc Hold'em."""
    # Set seed
    set_seed(args.seed)
    ...
    run_dir = Path(f"runs/leduc/dueling_polyak/{run_name}")
    ...
    # Initialize Dueling DQN agent with Polyak updates
    dqn_agent = DQNAgentDuelingPolyak(
    ...
    print(f"Dueling architecture + Polyak updates (tau={args.tau})")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Dueling DQN with Polyak updates on Leduc Hold'em"
    )
    ...
    # Polyak params
    parser.add_argument(
        "--tau", type=float, default=0.995, help="Polyak averaging coefficient"
    )
