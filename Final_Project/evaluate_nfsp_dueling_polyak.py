"""Standalone evaluation script for NFSP Dueling+Polyak agent (using average policy)."""
...
def evaluate_agent(checkpoint_path, num_episodes=1000, seed=42, agent_name="player_0"):
    """
    Evaluate NFSP agent (using average policy) against rule-based opponent.

    Args:
        checkpoint_path: Path to saved model checkpoint (without .pt extension)
        num_episodes: Number of evaluation episodes
        seed: Random seed
        agent_name: Name of agent in environment
    ...
    # Initialize and load NFSP agent with Dueling + Polyak
    agent = NFSPAgentDuelingPolyak(
        state_size=state_size,
        action_size=action_size,
        epsilon=0.0,  # Not used during evaluation
    )
    ...
    print("Using average policy (greedy) for NFSP agent")
    ...
    for ep in range(num_episodes):
        ...
        for agent_iter in env.agent_iter():
            ...
            if termination or truncation:
                ...
                if agent_iter == agent_name:
                    # Use average policy greedily (training=False)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate NFSP Dueling+Polyak agent on Leduc Hold'em"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
