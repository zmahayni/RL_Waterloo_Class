"""Training script for DQN on MiniGrid FourRooms with stacked observations (frame-based)."""
...
from dqn_agent_minigrid_stacked import DQNAgentMinGridStacked
...
def train_dqn(args):
    """Train DQN agent on MiniGrid FourRooms (frame-based)."""
    # Set seed
    set_seed(args.seed)
    ...
    epsilon_step = (args.epsilon_start - args.epsilon_end) / epsilon_decay_frames
    ...
    while total_frames < args.total_frames:
        ...
        while not done and total_frames < args.total_frames:
            ...
            episode_len += 1
            total_frames += 1
            agent.frame_count = total_frames

            # Update epsilon (frame-based decay)
            if total_frames < epsilon_decay_frames:
                new_epsilon = args.epsilon_start - epsilon_step * total_frames
...
def main():
    parser = argparse.ArgumentParser(
        description="Train DQN on MiniGrid FourRooms with stacked observations"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Target network update frequency (in frames)",
    )
    ...
    # Evaluation
    parser.add_argument(
        "--eval_every_frames", type=int, default=20000, help="Evaluate every N frames"
    )
    parser.add_argument(
        "--num_eval_episodes",
        type=int,
