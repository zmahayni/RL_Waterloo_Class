"""Training script for N-step DQN on MiniGrid FourRooms with stacked observations (frame-based)."""
...
def train_dqn(args):
    """Train N-step DQN agent on MiniGrid FourRooms (frame-based)."""
    # Set seed
    set_seed(args.seed)
    ...
    run_dir = Path(f"runs/minigrid/fourrooms/n_step/{run_name}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train N-step DQN on MiniGrid FourRooms with stacked observations"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="N-step for N-step returns (default: 3)",
    )
